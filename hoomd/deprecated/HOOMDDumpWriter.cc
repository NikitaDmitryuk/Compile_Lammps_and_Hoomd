...
void HOOMDDumpWriter::writeFile(std::string fname, unsigned int timestep)
    ...
    ofstream f(fname.c_str(),std::ofstream::app);
    ...
    if (m_output_position)
        ...
        for (unsigned int group_idx = 0; group_idx < N; ++group_idx)
            ...
            auto it2 = map.find(tag);
            vec3<Scalar> pos = snapshot.pos[tag];
            vec3<Scalar> vel = snapshot.vel[tag];
            //vec3<Scalar> accel = snapshot.accel[it->second];
            //vec3<Scalar> accel2 = snapshot.accel[tag];
            //vec3<Scalar> vel2 = snapshot.vel[it->second];
            //Scalar charge1 = snapshot.charge[it2->second];
            //Scalar charge2 = snapshot.charge[tag];
            //vec3<Scalar> accel = snapshot.accel[it->second];

            //f << pos.x << " " << pos.y << " "<< pos.z << "\n";
            f<< tag << " " << pos.x << " " << pos.y << " "<< vel.x << " " << vel.y << "\n";
            //f<< tag << " " << pos.x << " " << pos.y << " "<< vel.x << " " << vel.y << " " << vel.x*cos(charge1) + vel.y*sin(charge1) <<  "\n";
            //f<< tag << " " << pos.x << " " << pos.y << " " << " "<< accel.x << " " << accel.y << " " << "\n";
        //f<< tag << " " << pos.x << " " << pos.y << " " <<  vel.x << " " << vel.y << " " << charge << " " << charge<< " "<< accel.x<< " " << accel.y << "\n";
        //f<< tag<< " " << it->second << " " << charge1 << " " << charge2 << " " << vel2.x << " " << vel2.y << " " <<  vel.x << " " << vel.y << "\n";
        //f<< accel.x << " " << accel2.x << " " << cos(charge1) << "\n";
        //f<< accel.y << " " << accel2.y << " " << sin(charge1) << "\n\n";
            if (!f.good())
                {
...
void HOOMDDumpWriter::analyze(unsigned int timestep)
    ...
    if (m_mode_restart)
        ...
        string filetype = ".lammpstrj";

        // Generate a filename with the timestep padded to ten zeros
        full_fname << m_base_fname << filetype;
